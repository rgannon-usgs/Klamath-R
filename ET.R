...
ETdat <- lapply(ETdatfiles, read.csv) %>%
  rbindlist(fill = TRUE) %>% 
  mutate(ET = et/304.8, #"et" is in mm, converted here to ft ; `ET.Volume..ac.ft.` (previous volume)
         date = as.Date(date)) %>% 
  filter(date < "2020-10-01") %>% 
  select(OPENET_ID, date, ET)
...
ET_int <- st_read(ET_shp_intersect, quiet = T) %>% 
  st_transform(crs = crsnad) %>% 
  filter(!st_is_empty(geometry)) %>% 
  mutate(grid_area = Shape_Area,
         intersect_area = Area2,
         type = "shallowGW") %>% 
  select(OPENET_ID, SEQNUM, ROW, COLUMN_, grid_area, intersect_area, type, geometry) %>% 
...
ET_int2 <- st_read(ET_shp_intersect2, quiet = T) %>% 
  st_transform(crs = crsnad) %>% 
  filter(!st_is_empty(geometry)) %>% 
  mutate(grid_area = 580644,
         intersect_area = Shape_Area,
         type = "wetlands") %>% 
  select(OPENET_ID, SEQNUM, ROW, COLUMN_, grid_area, intersect_area, type, geometry) %>% 
  left_join(st_drop_geometry(ET_area2)) %>% 
  mutate(field_frac = intersect_area/field_area,
         cell_frac = intersect_area/grid_area)
...
ETlist <- dr_grid(ETchunks, grid, folder, title, "FPS")
...
#ETlist1 <- dr_grid(ETchunks1, grid, folder, title1, "FPS")
#ETlist2 <- dr_grid(ETchunks2, grid, folder, title2, "FPS")
...
ET_plot <- ggplot() +
  ...
  geom_sf(data = ET_area2, aes(color = type, fill = type), alpha = 1/3) +
